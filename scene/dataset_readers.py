...
def readColmapSceneInfo(path, images, eval, lod, llffhold=8):
    ...
    # cameras_extrinsic_file = os.path.join(path, "sparse/0", trajectory)
    # cam_extrinsics = read_extrinsics_binary(cameras_extrinsic_file)
    # cam_infos_trajectory = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=os.path.join(path, reading_dir))
    # folder_path = os.path.join(path, "sparse/0")
    # bin_path = bin_files = glob.glob(os.path.join(folder_path, 'images*.bin'))
    # cam_infos_trajectory = {}
    # for bin_file in bin_files:
    #     print(f'bin_file: {bin_file}')
    #     key = bin_file.split('/')[-1]
    #     cam_extrinsics = read_extrinsics_binary(bin_file)
    #     cam_infos_bin = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=os.path.join(path, reading_dir))
    #     cam_infos_trajectory[key] = cam_infos_bin
    if eval:
        if lod>0:
